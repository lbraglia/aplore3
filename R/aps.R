...
#' ## Table 8.2 p. 274
#' if (require(nnet)) {
#'   levels(aps$place3) <- abbreviate(levels(aps$place3))
#'   modt8.2 <- multinom(place3 ~ viol, data = aps)
#'   print(summary(modt8.2))
#'   print(exp(coef(modt8.2)[, "violYes"]))
#'   print(t(exp(confint(modt8.2)["violYes", ,])))

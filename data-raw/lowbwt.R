...
lowbwt <- within(lowbwt, {
  low <- factor(low, levels = zo, labels = low.label)
  race <- factor(race, levels = 1:3, labels = race.label)
  smoke <- noYes(smoke)
  ptl <- factor(ifelse(ptl > 2, 2, ptl), levels = 0:2, labels = no.one.twoetc)
  ht <- noYes(ht)
  ui <- noYes(ui)
  ftv <- factor(ifelse(ftv > 2, 2, ftv), levels = 0:2, labels = no.one.twoetc)
})
